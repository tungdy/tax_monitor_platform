...
合思报销单明细接口地址为 `https://116.63.221.181/post/hesimingxi`，使用独立 AppKey/AppSecret。Body 参数 `company_code`（公司代码编码）和 `submit_date`（提交时间）可选，提供时不可为空；`limitValue` 和 `offsetValue` 必填并由连接器按每页 `25` 行从 `offsetValue=0` 连续翻页至空终止页，避免网关大页响应漏数。无票计算在分页完成后对完全相同的报销明细行去重，并把去重数量写入批次证据。真实字段合同使用 `expense_code`、`flow_end_date`、`fee_type_code` 和 `fee_type_amount`。

“合思报销单发票查询”接口地址为 `https://116.63.221.181/post/hesiinvoice`，使用独立 AppKey/AppSecret。截图中的 API 目录 `/agent/jyfin` 是管理分组，实际请求 Path 为 `/post/hesiinvoice`；接口使用签名 POST 请求，`company_code`、`limitValue` 和 `offsetValue` 放入 JSON 请求体。连接器固定使用 `25` 行小页并连续递增 offset，直至空终止页；Hesi 专用严格模式不会因非空短页提前结束，并拒绝重复 offset。无票计算要求 `invoice_id`，分页完成后只按完整载荷 SHA-256 删除完全相同的重复行并写入批次证据；同一 `(code, invoice_id)` 的不同载荷视为发票分摊明细全部保留，不再误判为分页冲突。真实字段合同使用 `code`、`invoice_id`、`feetypeid`、`amount_standard_dec` 和 `approve_amount_dec`。

合思无票报销金额按公司、本年1月1日至季度末的审批完成时间聚合。合思报销明细排除以下费用类型编码：`CLF0101`、`CLF0102`、`CLF0103`、`CLF0117`、`CLF0118`、`CLF0119`、`CLF0120`、`CLF0126`、`CLF0130`、`CLF0131`、`F0507`、`F0508`、`F0605`、`F5409`、`F5724`、`F5725`、`F5809`、`F5811`、`F6309`；同时排除以 `F39`、`F72`、`F73`、`F35`、`F64`、`F07`、`F74`、`F44`、`F40`、`F55` 开头的全部费用类型编码。费用类型比较统一转为大写。发票先通过 `hesiinvoice.code = hesimingxi.expense_code` 关联报销单，再在同一报销单内用 `amount_standard_dec = fee_type_amount` 及稳定的 `feetypeid` 唯一识别 `fee_type_code`；无法唯一识别时阻断计算。未审批单据不计入。通常按“报销单号 + 报销类型”分别汇总 `fee_type_amount` 与 `approve_amount_dec` 并计算 `max(报销金额 - 发票核准金额, 0)`；如果同一报销单内任一 `invoice_id` 关联多个 `feetypeid`，则该报销单改为按 `code` 汇总所有未排除费用类型的报销金额和发票核准金额，再计算 `max(报销合计 - 发票合计, 0)`，避免同一张发票的跨费用类型分摊被重复截断。金额使用精确十进制。受控导入端点为 `POST /api/v1/ingest-batches/dgc-hesi-no-invoice`。
